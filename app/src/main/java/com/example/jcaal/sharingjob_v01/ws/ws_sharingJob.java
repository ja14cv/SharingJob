...
//
// Date Of Creation: 3/31/2015 4:41:11 AM
...
public class ws_sharingJob {
    ...
    public void verificar_correoAsync(String valor_correo) throws Exception{
        if (this.eventHandler == null)
            throw new Exception("Async Methods Requires IWsdl2CodeEvents");
        verificar_correoAsync(valor_correo, null);
    }
    
    public void verificar_correoAsync(final String valor_correo,final List<HeaderProperty> headers) throws Exception{
        ...
        new AsyncTask<Void, Void, String>(){
            ...
            @Override
            protected String doInBackground(Void... params) {
                return verificar_correo(valor_correo, headers);
            }
            @Override
            protected void onPostExecute(String result)
            {
                eventHandler.Wsdl2CodeEndedRequest();
                if (result != null){
                    eventHandler.Wsdl2CodeFinished("verificar_correo", result);
                }
            }
        }.execute();
    }
    
    public String verificar_correo(String valor_correo){
        return verificar_correo(valor_correo, null);
    }
    
    public String verificar_correo(String valor_correo,List<HeaderProperty> headers){
        SoapSerializationEnvelope soapEnvelope = new SoapSerializationEnvelope(SoapEnvelope.VER11);
        soapEnvelope.implicitTypes = true;
        soapEnvelope.dotNet = true;
        SoapObject soapReq = new SoapObject("ns:sharingJob","verificar_correo");
        soapReq.addProperty("valor_correo",valor_correo);
        soapEnvelope.setOutputSoapObject(soapReq);
        HttpTransportSE httpTransport = new HttpTransportSE(url,timeOut);
        try{
            if (headers!=null){
                httpTransport.call("ns:sharingJob/verificar_correo", soapEnvelope,headers);
            }else{
                httpTransport.call("ns:sharingJob/verificar_correo", soapEnvelope);
            }
            Object retObj = soapEnvelope.bodyIn;
            if (retObj instanceof SoapFault){
                SoapFault fault = (SoapFault)retObj;
