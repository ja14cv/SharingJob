...
/**
 * Created by jCaal on 19/05/2015.
 */
public class empleos_aplicados extends FragmentGenerico implements IWsdl2CodeEvents
{
    @Override
    ...
    public void hacerOnCreate() {
        lv_empleos = (ListView) getView().findViewById(R.id.empAp_lv_empleos);
        ...
        dialogo1.setTitle("Empleo aplicados");
        ...
        try {
            new ws_sharingJob(this).get_empleos_aplicadosAsync("{\"sesion\":\"" + sesion.id_sesion + "\"}");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void removeItem(int posicion){
        String json = "{\n" +
                ...
                "}";
        try {
            new ws_sharingJob(this).del_empleo_aplicadoAsync(json);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void Wsdl2CodeFinished(String methodName, Object Data) {
        ...
        if (methodName.equals("get_empleos_aplicados")){
        ...
        if (methodName.equals("del_empleo_aplicado")){
            switch(procesarData((String) Data)){
    ...
    private void procesar_empleos(String _json){
        ...
        try {
            ...
            for (int i=0 ; i<temp.length() ; i++){
                ...
                if (temporal.getString("ganador").equals("0")){
